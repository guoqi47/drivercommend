...
class ActivityDao(object):
    ...
    def valid(cls):
        ...
        :return:
        """
        return cls.Session.query(Activity).filter(cls.Activity.status==cls._status_for_handle)

    def getList(cls, now,activity_ids, isin):
        """
        ...
        base = cls.valid().filter(Activity.start_time < now, Activity.end_time > now - 400)
    ...
    def getListForConfStrategy(cls,now,city_ids):
        """
        ...
        for city_id in city_ids:
            ...
            acts= cls.getList(now,activityids,1)
            for a in acts:
               a.city_id = city_id
               ListAll.append(a)
        return ListAll
    ...
    @classmethod
    def getListForYearAndArea(cls,now,city_ids):
        """
        ...
        ListAll=cls.getList(now,activityids,0 ) #获取有效活动
        return ListAll
